...
class MFGFlowSpec extends FlatSpec with Matchers with ScalaFutures {
  ...
  "MFGFlow zipWithIndex" should "zipWithIndex a small stream" in {
  ...
  "MFGFlow byteStringToLines" should "convert a small byte stream to a string stream chunked by lines" in {
  ...
  it should "convert a big byte stream to a string stream chunked by lines" in {
    ...
    val futLines: Future[Seq[String]] =
      stream.via(FlowExt.rechunkByteStringBySeparator(maximumChunkBytes = 5 * 1024)).map(_.utf8String).runWith(SinkExt.collect)
  ...
  "MFGFlow rechunkByteString" should "rechunk a stream of bytes to bigger chunks" in {
    val stream = SourceExt.fromFile(bigFile, maxChunkSize = 1024)
  ...
  "MFGFlow rateLimiter" should "rate limit a downstream" in {
  ...
  "MFGFlow customConflate" should "conflate a stream according to a user defined function" in {
  ...
  it should "expand a stream according to a user defined function" in {
    val futSeq = Source(1 to 97).via(
      FlowExt.customStatefulProcessor[Int, Unit, Int](()) { // deduplicator
        (_, elem) => (Option(()), Vector(elem, elem))
      }
    )
    .runWith(SinkExt.collect)
